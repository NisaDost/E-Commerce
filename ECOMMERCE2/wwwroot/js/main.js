(function ($) {
    ...
    $('.quantity button').on('click', function () {
        ...
        var stockQuantity = button.parent().parent().find('input').attr('max');
        var newVal;
        if (button.hasClass('btn-plus') && oldValue < stockQuantity) {
            newVal = parseFloat(oldValue) + 1;
        } else if (button.hasClass('btn-plus') && oldValue == stockQuantity) {
            newVal = stockQuantity;
        } else {
            if (oldValue > 1) {
                newVal = parseFloat(oldValue) - 1;
            } else {
